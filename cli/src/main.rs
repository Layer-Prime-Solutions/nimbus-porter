//! Porter — standalone MCP gateway for MCP servers.
//!
//! Two subcommands:
...
use nimbus_porter::{PorterConfig, PorterMcpServer, PorterRegistry, run_hot_reload};
use rmcp::ServiceExt;
...
#[command(
    name = "porter",
    version,
    about = "Porter — standalone MCP gateway for MCP servers"
)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
    ...
    for &(top_row, bot_row) in &row_pairs {
        ...
        for x in 0..total_w {
            ...
            match (top, bot) {
                ...
                (Some(tc), None) => {
                    let _ = write!(buf, "\x1b[38;2;{};{};{}m▀▀\x1b[0m", tc.0, tc.1, tc.2);
                }
                (None, Some(bc)) => {
                    let _ = write!(buf, "\x1b[38;2;{};{};{}m▄▄\x1b[0m", bc.0, bc.1, bc.2);
                }
                (None, None) => {
                    buf.push_str("  ");
                }
